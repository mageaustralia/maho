...
class Mageaustralia_Storefront_Model_Observer
{
    ...
    /**
     * Inject storefront worker config into StoreConfig extensions.
     */
    public function onStoreConfigBuild(\Maho\Event\Observer $observer): void
    {
        ...
        // Google Maps API key is safe for public — restricted by referrer in Google Console
        $googleMapsKey = \Mage::getStoreConfig('mageaustralia_storefront/worker/google_maps_key');
        if ($googleMapsKey) {
            $dto->extensions['googleMapsKey'] = $googleMapsKey;
        }
    }
