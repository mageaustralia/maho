...
describe('Product Tier Prices — Permission Enforcement', function (): void {
    ...
    it('denies tier price delete without correct permission', function (): void {
        ...
        $response = apiDelete("/api/rest/v2/products/{$productId}/tier-prices", $token);
        expect($response['status'])->toBeForbidden();
    });
...
describe('Product Tier Prices — CRUD Lifecycle', function (): void {
    ...
    it('sets tier prices, reads them back, then removes them', function (): void {
        ...
        // Set tier prices
        $set = apiPut("/api/rest/v2/products/{$productId}/tier-prices", [
        ...
        // Read back
        $read = apiGet("/api/rest/v2/products/{$productId}/tier-prices");
        ...
        // Remove all
        $delete = apiDelete("/api/rest/v2/products/{$productId}/tier-prices", $token);
        expect($delete['status'])->toBeIn([200, 204]);

        // Verify empty
        $empty = apiGet("/api/rest/v2/products/{$productId}/tier-prices");
    ...
    it('replaces existing tier prices with new ones', function (): void {
        ...
        $set1 = apiPut("/api/rest/v2/products/{$productId}/tier-prices", [
            ['customerGroupId' => 'all', 'qty' => 3, 'price' => 29.95],
        ], $token);
        expect($set1['status'])->toBe(200);

        // Replace with different tier prices
        $set2 = apiPut("/api/rest/v2/products/{$productId}/tier-prices", [
        ...
        // Cleanup
        apiDelete("/api/rest/v2/products/{$productId}/tier-prices", $token);
    });

    it('returns 404 for non-existent product', function (): void {
        $response = apiGet('/api/rest/v2/products/999999/tier-prices');
